...
*/
#include <stdlib.h>
#include <string.h>//090616 in for memset() & strlen()
#include <gtk/gtk.h>
#include <time.h>
...
static void* vtm;
static void* vtm2;
static tParamGUI* p = NULL;
//static GdkPixmap *pixmap = NULL;

static int idle_func_winTkb32clGUI(void* p);//invoked by GTK, gtk_timeout_add(intval, idle_func_winTkb32clGUI, (void*)&p);
static int idle_func_winAllStatus(void* p);//invoked by GTK, gtk_timeout_add(intval, idle_func_winAllStatus, (void*)&p);


tParamGUI* callbacksSubInit(int argc, char* argv[]){
  unsigned int intval = 200;
  int i = 0;

  p = (tParamGUI*)malloc(sizeof(tParamGUI));
  memset((void*)p, 0, sizeof(tParamGUI));
  p->argc = argc;
  p->argv = argv;
...
int dialog_check(const char* message){//090616 in
/* 090616 out
