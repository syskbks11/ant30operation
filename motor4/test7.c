...
#define DEV_NAME    "/dev/ttyUSB2"        // デバイスファイル名
...
/* this supress the hard limit signal
...
int prepareAbsolute(int fd){
	...
	//buffer[len] = 0;
	printf("[MASK]%s",buffer);
    buffered_write(fd, "DIMAS,1=0\r", 10);//正転禁止のハードリミットを可能に
	len=getPreviousCommand(fd, buffer);
	//buffer[len] = 0;
	printf("[DIMAS,1=0]%s",buffer);
    buffered_write(fd, "DIMAS,2=0\r", 10);//逆転禁止のハードリミットを可能に
	len=getPreviousCommand(fd, buffer);
	//buffer[len] = 0;
	printf("[DIMAS,2=0]%s",buffer);
...
int movePositive(int fd){
	char buffer[BUFF_SIZE];    // データ受信バッファ

	setPulse(fd, 640000);//El
//	setPulse(fd, 7000000);//Az
	int len=getPreviousCommand(fd, buffer);
	buffer[len] = 0;
	printf("[P=6100000]%s",buffer);
	while(getPulseActual(fd) < 0){
		usleep(3000*1000);
	}
	int res;
	while(1){
...
int moveNegative(int fd){
	char buffer[BUFF_SIZE];    // データ受信バッファ

	setPulse(fd, -1700000);//El
//	setPulse(fd, -7000000);//Az
	int len=getPreviousCommand(fd, buffer);
	buffer[len] = 0;
	printf("[P=-7000000]%s",buffer);
	while(getPulseActual(fd) > 0){
		usleep(3000*1000);
	}
	int res;
...
//	limitState(fd, sn);
	...
	case 'X':
		...
		break;
	case 'h':
		setParameter(fd, 30, 1);
		len=getPreviousCommand(fd, buffer);
		buffer[len] = 0;
		printf("[#30=]%s\n", buffer);
		res = getParameter(fd, 30);
		printf("[#30]%d\n",res);
...
int main(int argc,char *argv[])
{
    ...
    fd = open(devName,O_RDWR | O_NOCTTY);//080905 in; 読み書き可能で開く、tty制御をしない。
