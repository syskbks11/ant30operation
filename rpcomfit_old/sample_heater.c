...
int main(int argc, char *argv[]) {
  ...
  if( Socket != -1 ) {
    ...
    memset(dataWord, 0x00, 20);
    if(HEADER!=0x00 && TRAILER!=0x00)
      sprintf(dataWord,"%c%.2dR%c%c%c%c", HEADER,atoi(argv[2]),argv[3][0],argv[3][1],argv[3][2],TRAILER);
    if(HEADER==0x00 && TRAILER!=0x00)
...
void DataSendReceive(int deviceid, char* dataWord, WORD dataSize, char* recvWord){
  ...
  while(i<10) {
    ...
    sprintf(recvWord+offset,(char*)pRxData,pRxStrmHead->wSTRMSize);
    *(recvWord + offset + pRxStrmHead->wSTRMSize) = 0x00;
    offset = offset + pRxStrmHead->wSTRMSize;
  ...
  if( TRAILER != 0x00 && *(recvWord + offset-1) != TRAILER ) {
    printf("ERROR:: Invalid Trailer: 0x%02x (should be 0x%02x)\n", *(recvWord + offset-1), TRAILER);
  }
