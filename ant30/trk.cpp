...
void* _trkThrdMain(void* _p){
  ...
  while(p.thrdRun){
        ...
        if(p.rastSofNo < p.rastSofSize){
          ...
          */
          ...
          while(tmGetDiff(p.rastStartTime, offsetMSec) > 0.001){
            if(controlBoardGetAntenna() == CONTROL_BOARD_USE_FULL){//090629 for 30 cm
              ret = acuGet();
              if(ret){
                uM1("_trkThrdMain(); acuGet(); ret = %d", ret);
                p.thrdRet = ret;
              }
    ...
    /*** 2011/01/26 in ***/
    //acuGetRAZEL(acuStatDazel);
    //uM2("_trkThrdMain(); real(AZ,EL) = (%f,%f)", acuStatDazel[0], acuStatDazel[1]);//debug
    ...
    if(controlBoardGetAntenna() == CONTROL_BOARD_USE_FULL){//090611 in
      ...
      */
      ...
      if(ret){
        ...
        ret = acuSet(p.antZone);
        if(ret){
          uM1("_trkThrdMain(); acuSet(); return %d", ret);
          p.thrdRet = ret;
        }
      }
    }//!< if(p.TrkUse==1)
  ...
  while(p.thrdStatusRun){
    ...
    if(controlBoardGetAntenna() == CONTROL_BOARD_USE_FULL){//090611 in
      ...
      */
      ret = acuGet();
      if(ret){
	uM2("_trkThrdStatus(); acuGet(); [%d(%s)]", ret, errnoStr(ret));
	p.thrdRet = ret;
      }
      ret = acuSafetyCheck();
    }
...
void _trkCalTrackStatus(const int reset){
  ...
  */
  ...
  //uM2("(dAz, dEl)=(%.2lf\",%.2lf\")", p.deazel[0]*3600, p.deazel[1]*3600);
