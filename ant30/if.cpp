...
typedef struct sReqIf1{
	...
	double lo1amp;     //!< 1st Local Amplitude [dBm]
	double att;       //!< 1st IF Step Att [-dB]
/*
...
*/
    ...
    if(fabs(p.aIf1.ansAtt1 - p.req1.att) >= _LO1_ATT_THRESH){
      uM2("ifRepeat(); StepAtt req[%.2lf] ans[%.2lf] error!!",
	  p.aIf1.ansAtt1, p.req1.att);
      ret = IF_STATE_ERR;
    }
  }
	if(controlBoardGet2ndIF() == CONTROL_BOARD_USE_FULL){//090611 in
...
*/
	...
	//30-cm telescope
	p.req1.lo1freq      = (calc2GetXoutDflkfq() / 1.0e9 - p.FifL)/108.0; //!< [GHz]
/* Tsukuba 32-m telescope
	p.req1.lo1freq      = calc2GetXoutDflkfq() / 1.0e9 - p.FifL; //!< [GHz]
*/
/*
	p.req1.lo1freq      = p.xout.dflkfq / 1.0e9 - p.FifL; //!< [GHz]
*/
	p.req2.lo2freq      = p.FifL - p.fcenter; //!< [GHz]
...
*/
  uM3("Request Lo1(%.9lf GHz, %.1lf dBm), Att %4.1lf dB", p.req1.lo1freq, p.req1.lo1amp, p.req1.att);
...
/*
  rIf1.reqLo1Freq = p.req1.lo1freq;         //!< [GHz]
  rIf1.reqLo1Amp  = p.req1.lo1amp;          //!< [dBm]
  rIf1.reqAtt1    = p.req1.att;             //!< [dB]
  rIf2.reqLo2Freq = p.req2.lo2freq;         //!< [GHz]
  rIf2.reqLo2Amp  = p.req2.lo2amp;          //!< [dBm]
*/
  err = 0;
  //! 第一中間周波数部制御プログラム
	if(controlBoardGet1stIF() == CONTROL_BOARD_USE_FULL){//090611 in
...
void ifSetLo1Att(double Att){
  ...
  uM2("ifSetLo1Att(); Set p.req1.att = %lf (OnOffR = %d)", Att, p.OnOffR);
...
int ifSetAttOnOffR(int OnOffR){
  ...
  uM2("ifSetAttOnOffR(); Set OnOffR=%d StepAtt=%.1lf [dB]", OnOffR, p.req1.att);
  return 0;
}
...
/* 090611 out
  if(p.use1 != 0){
*/
    //if(p.req1.lo1amp < -30 || p.req1.lo1amp > 10)
    //  return -1;
    //if(p.req1.att1 < -11 || p.req1.att1 > 0){
    //  uM2("useIf1(%d) Att_H(%d) error!!¥n", p.use1, p.req1.att);
    //  return -1;
    //}
  }
...
/* 090611 out
  if(p.use2 != 0){
*/
    //if(p.req2.lo2amp < -30 || p.req1.lo1amp > 10)
    //  return -1;
  }
