...
int obsStop(){
  if(p.thrdObsRun){
    //beReqStop();
    p.thrdObsRun = 0;
    uM("obsStop(); for debug before obsAuto join");// 080919 in
    pthread_join(p.thrdObsId, NULL);
    uM("obsStop(); for debug after obsAuto join");// 080919 in
  }
  //uM("obsStop(); for debug");// 080919 in
  return 0;
}
...
void* obsAuto(void* _p){
  ...
  //! 観測終了処理
  uM("obsAuto(); for debug trkStop() before");// 080920 in
  trkStop();
  uM("obsAuto(); for debug trkStop() after");// 080920 in
  p.thrdObsRun = 0;
  p.reqRet = 0;  
  uM("### End observation. ");
...
int getAnsData(unsigned char** ans, int *ansSize){
  ...
  long size;
  time_t t;
  //double msec;// 080825in

  if(p.req->reqFlg==0){
    //! Status
    time(&t);//080825 out
    //tmGetTime(0, &t, &msec);//080825 in
    p.stat.UT = t;
    //p.stat.ms = msec;//080825 in
