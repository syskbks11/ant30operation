...
int trkInitParam(){
  ...
  if(p.ScanFlag >= 1 && p.ScanFlag <= 6){
	...
	if(p.mapPattern[k] > p.OnNumber){
	  uM1("trkInitParam(); invalid SetPattern[%d]", p.SetPattern[k]);
	  p.mapPattern[k] = 0;
	}
	else{
	  k++;
	...
	uM1("trkInitParam(); invalid SetPattern data [%c]", p.SetPattern[i]);
      }
...
int trkStop(){
  ...
  if(p.thrdRun){
    p.thrdRun=0;
    uM("trkStop(); for debug before trkThrdMain join");// 080929 in
    tmSleepMSec(1000); //!< trkSetSofNext();直後にtrkGetTrackStatus();を行うと、直前のTrackingを出力してしまうため1.0[sec]待つ 080929 in
    uM("trkStop(); for debug before trkThrdMain join");// 080920 in
    pthread_join(p.thrdID, NULL);//hagiwara
    uM("trkStop(); for debug after trkThrdMain join");// 080920 in
  }
  else{
    return TRK_THRD_STOPPED_ERR;
  }
  uM("trkStop(); for debug before return");// 080920 in
  return 0;
}
...
void* trkThrdMain(void* _p){
  ...
  while(p.thrdRun){
	...
	if(p.rastSofNo < p.rastSofSize){
	  ...
	  p.rastStart = 0;
	  p.rastSofNo = 0;
	  uM("trkThrdMain(); p.rastStart = 0 done");// 080920 insert
    ...
    //uM("trkThrdMain(); for debug before if(p.TrkUse == 0)");//080920 in
    if(p.TrkUse == 1){
      ret = trkSafetyCheck();
      //! debug
      //! ACUへ送信
  ...
  uM("trkThrdMain(); for debug before p.thrdRun = 0");// 080920 in
  p.thrdRun = 0;
  //p.thrdRet = 0;
  uM("trkThrdMain(); for debug after p.thrdRun = 0");// 080920 in

  //! 現在のRealAZELで指令をかける。停止させる。
  if(p.TrkUse == 1){
    //uM("trkThrdMain(); for debug p.TrkUse == 1");// hagiwara
    ret = trkSafetyCheck();
    //uM("trkThrdMain(); for debug called trkSafetyCheck()");// hagiwara
    if(!ret){
      //uM("trkThrdMain(); for debug trkSafetyCheck() == 0");// hagiwara
      p.antZone = 0; //! 0:Auto
      p.doazelC[0] = p.acuStat.dazel[0];
      p.doazelC[1] = p.acuStat.dazel[1];
      ret = trkSetACU();
      //uM("trkThrdMain(); for debug called trkSetACU ");// hagiwara
      if(ret){
	uM1("trkThrdMain(); trkSetACU(); return %d", ret);
	p.thrdRet = ret;
      }
  ...
  /*else {
    uM1("trkThrdMain(); pthread_create(trkThrdStatus); status=%d error", status);// hagiwara
    }*/

  uM("trkThrdMain(); for debug before return");// 080920 in
  return NULL;
}
...
void* trkThrdStatus(void* _p){
  ...
  while(p.thrdStatusRun){
    ...
    if(cnt == 0) {
      cnt++;
      uM("trkThrdStatus(); for debug after tmReset()");// 080929 in
    } 
   //p.thrdRet = 0;
    if(p.TrkUse == 1){
      ret = trkGetACU();
...
int trkGetACU(){
  ...
  //uM("ACU Get Status Start");//for debug 080903 in
  ...
  //uM("ACU Get Status End");//for debug 080903 in
  //uM("ACU Read Status Start");//for debug 080903 in 080914 out

  //! ステータス読込み
  size=0;
  for(i=0; i<5; i++){
    //ret=rs232cRead(p.TrkCom, buf+size, sizeof(tAcuStat)-size);
    //uM("rs232cRead Start in for loop");//for debug 080913 in 080914 out
    ret=rs232cRead(p.TrkCom, buf+size, bufSize-size);
    //uM("rs232cRead End in for loop");//for debug 080913 in 080914 out
    if(ret<0){
      comAct=0;
      rs232cEnd(p.TrkCom);
      return TRK_COM_ERR;
      ...
      //uM("for loop break!");//for debug 080913 in 080914 out
      break;
    }
    tmSleepMSec(10);
  }
  //uM("ACU Read Status End");//for debug 080903 in 080914 out
  if(i==5){
    //buf[size] = '\0';
    //uM1("%s", buf);
