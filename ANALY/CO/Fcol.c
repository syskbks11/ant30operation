...
fprintf(stderr,"IN_FITS(12C),IN_FITS(13C) -> OUT_FITS(Ncol)\n");
...
for(ii=0;ii<data_num;ii++){
	...
	(Blank(fits1.data[ii])==(-1))||(Blank(fits2.data[ii])==(-1)))	
		...
		if((err_code==0)&&(tau>0.0)){
			err_code=Tex(nu1, (double)(fits1.data[ii]), tau, &tex);
			if((err_code==0)&&(tex>0.0)){
				ncol=4.65e12*tex*exp(33.2/tex)*fits1.data[ii]*tau/(1.0-exp(-tau));
				data_float = (float)ncol;
			}
