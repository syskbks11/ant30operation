...
extern	int Gaussian_Lorenzian_fit(SPECT *, GAUSS *);
...
err_code=Gaussian_Lorenzian_fit(&spect, &gauss);
if(err_code!=0){
...
for(ii=0;ii<spect.num;ii++){
	y = gauss.ag * exp(-pow(((spect.x[ii]-gauss.xg)/gauss.sg),2.0)) + gauss.al  /(1.0 + pow((spect.x[ii]-gauss.xl)/gauss.sl,2.0) ); 
...
Gaussian_Lorenzian_fit(spect, gauss)
SPECT	*spect;
GAUSS	*gauss;
{
int	i, j, ii, jj;
...
xg=(double)xc; xl=xg; 
sg=width/100.0; sl=sg;
dxg=sg/IT_FACT; dxl=dxg;
dsg=sg/IT_FACT; dsl=dsg;


for(i=0;i<MAX_ITERATION;i++){
ch_min=chc-(int)(sg*5.0/width*(double)(*spect).num);
ch_max=chc+(int)(sg*5.0/width*(double)(*spect).num); 
...
/*  Linear Part  */
	...
	for(ii=ch_min;ii<ch_max;ii++){
		...
		a[0][1]=a[0][0]+exp(-pow((((*spect).x[ii]-xg)/sg),2.0))*1.0/(1.0 + pow((((*spect).x[ii]-xl)/sl), 2.0));
		a[1][1]=a[1][1]+1.0/(1.0 + pow((((*spect).x[ii]-xl)/sl), 2.0))*1.0/(1.0 + pow((((*spect).x[ii]-xl)/sl), 2.0));
		b[0]=b[0]+exp(-pow((((*spect).x[ii]-xg)/sg),2.0))*(*spect).y[ii];
		b[1]=b[1]+1.0/(1.0 + pow((((*spect).x[ii]-xl)/sl), 2.0))*(*spect).y[ii];
	}
		a[1][0]=a[0][1];
...
/*  Non-linear Part  */
	I=0.0; Ip=0.0; Im=0.0; 
	for(ii=ch_min;ii<ch_max;ii++){
		I=I+pow(((ag*exp(-pow((((*spect).x[ii]-xg)/sg),2.0))+al/(1.0 + pow((((*spect).x[ii]-xl)/sl), 2.0)))-(*spect).y[ii]),2.0);
		Ip=Ip+pow(((ag*exp(-pow((((*spect).x[ii]-(xg+dxg))/sg),2.0))+al/(1.0 + pow((((*spect).x[ii]-xl)/sl), 2.0)))-(*spect).y[ii]),2.0);
		Im=Im+pow(((ag*exp(-pow((((*spect).x[ii]-(xg-dxg))/sg),2.0))+al/(1.0 + pow((((*spect).x[ii]-xl)/sl), 2.0)))-(*spect).y[ii]),2.0);
	}
	...
	I=0.0; Ip=0.0; Im=0.0; 
	for(ii=ch_min;ii<ch_max;ii++){
		I=I+pow(((ag*exp(-pow((((*spect).x[ii]-xg)/sg),2.0))+al*1.0/(1+pow(((*spect).x[ii]-xl)/sl,2.0)))-(*spect).y[ii]),2.0);
		Ip=Ip+pow(((ag*exp(-pow((((*spect).x[ii]-xg)/(sg+dsg)),2.0))+al/(1.0 + pow((((*spect).x[ii]-xl)/sl), 2.0)))-(*spect).y[ii]),2.0);
		Im=Im+pow(((ag*exp(-pow((((*spect).x[ii]-xg)/(sg-dsg)),2.0))+al/(1.0 + pow((((*spect).x[ii]-xl)/sl), 2.0)))-(*spect).y[ii]),2.0);
	}
	...
	for(ii=ch_min;ii<ch_max;ii++){
		I=I+pow(((ag*exp(-pow((((*spect).x[ii]-xg)/sg),2.0))+al*1.0/(1+pow(((*spect).x[ii]-xl)/sl,2.0)))-(*spect).y[ii]),2.0);
		Ip=Ip+pow(((ag*exp(-pow((((*spect).x[ii]-xg)/sg),2.0))+al/(1.0 + pow((((*spect).x[ii]-(xl+dxl))/sl), 2.0)))-(*spect).y[ii]),2.0);
		Im=Im+pow(((ag*exp(-pow((((*spect).x[ii]-xg)/sg),2.0))+al/(1.0 + pow((((*spect).x[ii]-(xl-dxl))/sl), 2.0)))-(*spect).y[ii]),2.0);
	}
	...
	for(ii=ch_min;ii<ch_max;ii++){
		I=I+pow(((ag*exp(-pow((((*spect).x[ii]-xg)/sg),2.0))+al*1.0/(1+pow(((*spect).x[ii]-xl)/sl,2.0)))-(*spect).y[ii]),2.0);
		Ip=Ip+pow(((ag*exp(-pow((((*spect).x[ii]-xg)/sg),2.0))+al/(1.0 + pow((((*spect).x[ii]-xl)/(sl+dsl)), 2.0)))-(*spect).y[ii]),2.0);
		Im=Im+pow(((ag*exp(-pow((((*spect).x[ii]-xg)/sg),2.0))+al/(1.0 + pow((((*spect).x[ii]-xl)/(sl-dsl)), 2.0)))-(*spect).y[ii]),2.0);
	}
