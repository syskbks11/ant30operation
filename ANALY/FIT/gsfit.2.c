...
/*  Linear Part  */
	...
	ag=b[0];  cg=b[1];
	if(ag<0.0) ag=fabs(ag);	if(cg<0.0) cg=fabs(cg);


/*  Non-linear Part  */
	I=0.0; Ip=0.0; Im=0.0; 
	for(ii=ch_min;ii<=ch_max;ii++){
		I=I+pow( ( ag + cg*exp(-pow(((*spect).x[ii]-xg)/sg,2.0)) - (*spect).y[ii] )*weight((*spect).x[ii],xg,sg),2.0);
		Ip=Ip+pow( ( ag + cg*exp(-pow(((*spect).x[ii]-(xg+dxg))/sg,2.0)) - (*spect).y[ii] )*weight((*spect).x[ii],xg,sg),2.0);
		Im=Im+pow( ( ag + cg*exp(-pow(((*spect).x[ii]-(xg-dxg))/sg,2.0)) - (*spect).y[ii] )*weight((*spect).x[ii],xg,sg),2.0);
	}
	...
	Ip=0.0; Im=0.0; 
	for(ii=ch_min;ii<=ch_max;ii++){
		Ip=Ip+pow( ( ag + cg*exp(-pow(((*spect).x[ii]-xg)/(sg+dsg),2.0)) - (*spect).y[ii] )*weight((*spect).x[ii],xg,sg),2.0);
		Im=Im+pow( ( ag + cg*exp(-pow(((*spect).x[ii]-xg)/(sg-dsg),2.0)) - (*spect).y[ii] )*weight((*spect).x[ii],xg,sg),2.0);
	}
