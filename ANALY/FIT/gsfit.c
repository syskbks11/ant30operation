...
/*  Linear Part  */
	...
	for(ii=ch_min;ii<=ch_max;ii++){
		...
		a[0][1] = a[0][1] + ((*spect).x[ii]-xg)*weight((*spect).x[ii],xg,sg);
		a[0][2] = a[0][2] + exp(-pow(((*spect).x[ii]-xg)/sg,2.0))*weight((*spect).x[ii],xg,sg);
		a[1][1] = a[1][1] + ((*spect).x[ii]-xg)*((*spect).x[ii]-xg)*weight((*spect).x[ii],xg,sg);
		a[1][2] = a[1][2] + ((*spect).x[ii]-xg)*exp(-pow(((*spect).x[ii]-xg)/sg,2.0))*weight((*spect).x[ii],xg,sg);			
		a[2][2] = a[2][2] + exp(-pow(((*spect).x[ii]-xg)/sg,2.0))*exp(-pow(((*spect).x[ii]-xg)/sg,2.0))*weight((*spect).x[ii],xg,sg);
		b[0] = b[0] + (*spect).y[ii]*weight((*spect).x[ii],xg,sg);
		b[1] = b[1] + ((*spect).x[ii]-xg)*(*spect).y[ii]*weight((*spect).x[ii],xg,sg);
		b[2] = b[2] + exp(-pow(((*spect).x[ii]-xg)/sg,2.0))*(*spect).y[ii]*weight((*spect).x[ii],xg,sg);
	}
		a[1][0] = a[0][1];	a[2][0] = a[0][2];	a[2][1] = a[1][2];
	err_code=sweepout(a,b,3,3,EPS);	
