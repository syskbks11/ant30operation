...
long	pos, posr;
float	lsep[MAX_DIMENSION], gsep[MAX_DIMENSION];
int	edge;


void	Gaussian_smooth(fits,fits_new,w,weight,blank)
FITS	*fits, *fits_new;
float	w[];
float	weight[];
short	blank[];
...
ch=(*fits_new).head.crval[cc+1]+(*fits_new).head.cdelt[cc+1]*((float)pix[cc]+1.0-(*fits_new).head.crpix[cc+1]);
	...
	for(pixr[cc]=min;pixr[cc]<=max;pixr[cc]++){
		cc++;
		Gaussian_smooth(fits,fits_new,w,weight,blank);
		cc--;
	}
}
else{
	{int	ii, flag;
	float	sep[MAX_DIMENSION];
	float	Exp;
	Exp=1.0;
	for(ii=0;ii<(*fits).head.naxis[0];ii++){	
		sep[ii] = ((*fits).head.crval[ii+1]+(*fits).head.cdelt[ii+1]*((float)pixr[ii]-(*fits).head.crpix[ii+1]+1.0)) 
			- ((*fits_new).head.crval[ii+1]+(*fits_new).head.cdelt[ii+1]*((float)pix[ii]-(*fits_new).head.crpix[ii+1]+1.0));
		if(w[ii]==0.0){
	...
	else{	(*fits_new).data[pos] = (*fits_new).data[pos] + (*fits).data[posr]*Exp;
...
void	Call_Gaussian_smooth(fits,fits_new,w,weight,blank)
FITS	*fits, *fits_new;
float	w[];
float	weight[];
short	blank[];
...
if(c < (*fits_new).head.naxis[0]){
	for(pix[cc]=0;pix[cc]<(*fits_new).head.naxis[cc+1];pix[cc]++){
		c++;
		Call_Gaussian_smooth(fits,fits_new,w,weight,blank);
		c--;
	}
}
else{
...
int	smooth_FitsData(fits, fits_new, w, g, anc)
FITS	*fits, *fits_new;
float	w[], g[], anc[];
{
int	i, j, k, ii, jj;
...
(*fits_new).head.naxis[i+1]=tint((float)(*fits).head.naxis[i+1]/g[i]*fabs((*fits).head.cdelt[i+1]));	
/*(*fits_new).head.naxis[i+1]=(int)floor((float)(*fits).head.naxis[i+1]/g[i]*fabs((*fits).head.cdelt[i+1]));	*/
	(*fits_new).head.cdelt[i+1]=fabs(g[i]/(*fits).head.cdelt[i+1])*(*fits).head.cdelt[i+1];
	(*fits_new).head.crval[i+1]=anc[i];
	(*fits_new).head.crpix[i+1]=tint(( (*fits).head.cdelt[i+1]*((*fits).head.crpix[i+1]-(float)(*fits).head.naxis[i+1]/2.0)
					+ (*fits_new).head.cdelt[i+1]*(float)(*fits_new).head.naxis[i+1]/2.0
...
for(ii=0;ii<(*fits_new).head.naxis[0]-1;ii++){
	fprintf(stderr,"%lf, ",g[ii]);
}
fprintf(stderr,"%lf)\n",g[(*fits_new).head.naxis[0]-1]);

fprintf(stderr,"Anchor position : (");
for(ii=0;ii<(*fits_new).head.naxis[0]-1;ii++){
	fprintf(stderr,"%lf, ",anc[ii]);
}
fprintf(stderr,"%lf)\n",anc[(*fits_new).head.naxis[0]-1]);
...
FITS		fits, fits_new;
int		ii, jj;
float		w[MAX_DIMENSION], g[MAX_DIMENSION], anc[MAX_DIMENSION];
int		dim;
char		in_filename[80];
...
for(jj=0;jj<fits.head.naxis[0];jj++){
	if(token==NULL){
		for(ii=jj;ii<fits.head.naxis[0];ii++){g[ii]=fabs(fits.head.cdelt[ii+1]); }
		break;
	}
	else	g[jj]=atof(token);
	token=strtok(NULL,", \t");	
}
...
fprintf(stderr,"\nSmoothing %s\n",in_filename);
err_code=smooth_FitsData(&fits, &fits_new, w, g, anc);
if(err_code!=0){
	fprintf(stderr,
