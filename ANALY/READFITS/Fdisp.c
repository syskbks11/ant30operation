...
int	calc_dispersions(fits, range, bound, disp, cent, peak, cosd, integ, peak_val)
FITS	*fits;
RANGE	range[];
double	disp[], cent[], peak[], cosd[];
...
for(ii=1;ii<=(*fits).head.naxis[0];ii++){
	min[ii-1]=tint((double)(*fits).head.crpix[ii] + 
		((double)range[ii-1].min-(*fits).head.crval[ii])/((*fits).head.cdelt[ii]/cosd[ii]));
	max[ii-1]=tint((double)(*fits).head.crpix[ii] + 
		((double)range[ii-1].max-(*fits).head.crval[ii])/((*fits).head.cdelt[ii]/cosd[ii]));
	...
	if(min[ii-1]<1){
		min[ii-1]=1;
		fprintf(stderr,"CH#%d minimum value falls below 1.\n",ii);
	}
	if(max[ii-1]>(*fits).head.naxis[ii]){
		max[ii-1]=(*fits).head.naxis[ii];
		fprintf(stderr,"CH#%d maximum value exceeds the naxis[%d].\n",ii,ii);
	}
}
...
for(ii=0;ii<(*fits).head.naxis[0];ii++){
	cent[ii]=x[ii]/w;
	disp[ii]=pow(x2[ii]/w-pow(x[ii]/w,2.0),0.5)*cosd[ii+1];
}
...
if((ra==(-1))&&(dec==(-1)))	declination=0.0;
else				declination = (min+max)/2.0;
...
err_code=calc_dispersions(&fits, range, bound, disp, cent, peak, cosd, &integ, &peak_val);
if(err_code!=0){
	fprintf(stderr,
