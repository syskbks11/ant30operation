...
if(cc+1==ra){
	...
	min = (int)((*fits).head.crpix[cc+1]-1.0+(ch-(*fits).head.crval[cc+1])/(*fits).head.cdelt[cc+1]*cos_dec)
		-(int)(RESTRICT*w[cc]/2.0/fabs((*fits).head.cdelt[cc+1])*cos_dec) -1; 	
	if(min<0) min=0;
	...
	min = (int)((*fits).head.crpix[cc+1]-1.0+(ch-(*fits).head.crval[cc+1])/(*fits).head.cdelt[cc+1])
		-(int)(RESTRICT*w[cc]/2.0/fabs((*fits).head.cdelt[cc+1])) -1; 	
	if(min<0) min=0;
...
if(i+1==ra){
	...
	lsep[i] = fabs(fabs((*fits_new).head.cdelt[i+1])-fabs((*fits).head.cdelt[i+1]))/2.0;
