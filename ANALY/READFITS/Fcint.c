...
int	integ_Fits(fits, crit, fp_out)
FITS	*fits;
double	crit;
FILE	*fp_out;
{
...
for(ch=0;ch<(*fits).head.data_num;ch++){
	if(Blank((*fits).data[ch])==(-1)){
		...
		if((*fits).data[ch]>crit) w=w+(double)(*fits).data[ch];
	}
}
...
fprintf(stderr,"Minimum intensity threshold :");
fscanf(stdin,"%lf",&crit);
...
err_code=integ_Fits(&fits, crit, fp_out);
if(err_code!=0){
	fprintf(stderr,
