...
//static const int srvPort = 10101;
//static netServerClass_t* net = NULL;

int _simpleLogger(){
  int ret;

  uInit("../log/srvHeater/srvHeater");//090813 in
  uM2("This program is Compiled at %s %s", __DATE__, __TIME__);

  ret = penguin_heater_init();

  if(ret){
    uM1("main(); penguin_heater_init(); return %d", ret);
    return -1;
  }
  while(1){
    double temp1, temp2, temp3;
    temp1 = penguin_heater_get_temperature(1);
    if(temp1 == -99999){//090206 in
      uM("CH1 failed.");
      temp1 = penguin_heater_get_temperature(1);
    }
    temp2 = penguin_heater_get_temperature(2);
    if(temp1 == -99999){//090206 in
      uM("CH2 failed.");
      temp2 = penguin_heater_get_temperature(2);
    }
    temp3 = penguin_heater_get_temperature(3);
    if(temp1 == -99999){//090206 in
      uM("CH3 failed.");
      temp3 = penguin_heater_get_temperature(3);
    }
