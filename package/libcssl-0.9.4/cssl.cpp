...
int _buffered_write(int fd, const void* buf, const unsigned int size);
...
/* sending a char */
int cssl_putchar(cssl_t *serial,
		     char c)
{
    if (!cssl_started) {
	cssl_error=CSSL_ERROR_NOTSTARTED;
    ...
    return _buffered_write(serial->fd,&c,1);
}

/* sending a null-terminated string */
int cssl_putstring(cssl_t *serial,
		     char *str)
{
    if (!cssl_started) {
	cssl_error=CSSL_ERROR_NOTSTARTED;
    ...
    return _buffered_write(serial->fd,str,strlen(str));
}
		  ...
		  int datalen)
{
    ...
    return _buffered_write(serial->fd,data,datalen);
}
...
/*
 * added by NAGAI Makoto. replaces all write().
 */
int _buffered_write(int fd, const void *buf, const unsigned int size)
{
