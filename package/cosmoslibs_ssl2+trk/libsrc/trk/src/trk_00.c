...
extern int trk_02 (double, double *, double *);
extern int trk_03_cmt (char *, char *, int *, double *, double *, double *, double *, char *);
extern void trk_01_ (double *, double *, double *, int *);
...
printf( "DMJD=%f\n", dmjdjst-dmjd );
  ...
  if (strlen(cjpl) != 0) {
    if (trk_03_cmt(cjstdate, cjpl, &t_cmt.imax,
      &t_cmt.ldmjd[0], &t_cmt.ldrad[0][0], &t_cmt.lddkm[0], &t_cmt.ldang[0], cerr) != 0) {
      return( 4 );
    }
  }
