...
tNetParam* netclInitWithTimeOut(const char* serverName, const int serverPort, const double t){
  ...
  if(ret < 0){
    uM3("netclInit(); Connection %s:%d error!! (ret = %d)", param->srvName, param->srvPort, ret);
    //netclEnd(param);
    param->sock = -1;
  }
  else{
...
int _netclConnect(tNetParam* param){
  ...
  if(connect(param->sock, (struct sockaddr*)&param->sockadd, sizeof(param->sockadd))==-1){
   ...
   uM3("_netclConnect(); Connection %s:%d, errno: %d error!!\n", param->srvName, param->srvPort, errno);
#ifdef WIN32
    closesocket(param->sock);
#else
    close(param->sock);
