...
int	device_no=-1;
        ...
        printf("[DEBUG] vxi11_open_device called with IP: %s, device: %s\n", ip, device ? device : "NULL");
    if (ip == NULL || strlen(ip) == 0) {
        printf("[ERROR] Invalid IP address provided.\n");
        return -1;
    }
    if (clink == NULL) {
        printf("[ERROR] clink is NULL at the start of vxi11_open_device.\n");
        return -1;
    }
//	printf("before doing anything, clink->link = %ld\n", clink->link);
	...
	for (l=0; l<VXI11_MAX_CLIENTS; l++){
		if (strcmp(ip,VXI11_IP_ADDRESS[l]) == 0 ) {
			device_no=l;
                       // printf("[DEBUG] Found existing IP: %s at index %d\n", ip, l);
//			printf("Open function, search, found ip address %s, device no %d\n",ip,device_no);
	...
	if (device_no < 0) {
		 ...
		 * opened so we don't run out of storage space. */
                        ...
                        printf("[DEBUG] Creating new client for IP: %s\n", ip);
...
//	printf("after creating link, clink->link = %ld\n", clink->link);
      //  printf("[DEBUG] vxi11_open_device returning with code: %d\n", ret);
      //  printf("[DEBUG] Final clink state: client=%p, link=%p\n", (void *)clink->client, (void *)clink->link);
	return ret;
	}
...
int	vxi11_send(CLINK *clink, const char *cmd) {
    if (clink == NULL) {
        printf("[ERROR] vxi11_send (vxill_user.cc 1): clink is NULL.\n");
        return -1;
    }
    if (cmd == NULL) {
        printf("[ERROR] vxi11_send (vxi11_user.cc 1): cmd is NULL.\n");
        return -1;
    }
...
int	vxi11_send(CLINK *clink, const char *cmd, unsigned long len) {
    if (clink == NULL) {
        printf("[ERROR] vxi11_send (vxi11_user.cc 2): clink is NULL.\n");
        return -1;
    }
    if (cmd == NULL) {
        printf("[ERROR] vxi11_send (vxi11_user.cc 2): cmd is NULL.\n");
        return -1;
    }
...
long	vxi11_receive(CLINK *clink, char *buffer, unsigned long len) {
    if (clink == NULL) {
        printf("[ERROR] vxi11_receive (vxi11_user.cc 1): clink is NULL.\n");
        return -1;
    }
    if (buffer == NULL) {
        printf("[ERROR] vxi11_receive (vxi11_user.cc 1): buffer is NULL.\n");
        return -1;
    }
...
long	vxi11_receive(CLINK *clink, char *buffer, unsigned long len, unsigned long timeout) {
    if (clink == NULL) {
        printf("[ERROR] vxi11_receive (vxi11_user.cc 2): clink is NULL.\n");
        return -1;
    }
    if (buffer == NULL) {
        printf("[ERROR] vxi11_receive (vxi11_user.cc 2): buffer is NULL.\n");
        return -1;
    }
	...
	do {
		...
		if (bytes_returned <= 0) {
			if (bytes_returned >-VXI11_NULL_READ_RESP) {
				...
				printf("       The function vxi11_receive returned %d. ",bytes_returned);
				return -2;
				}
