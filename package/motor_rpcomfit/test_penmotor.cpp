...
void showHelp(){
  ...
  printf("%s\n", "\tPTIMEATTACK [NUM]: ");
  printf("%s\n", "\t\trepeat PMON command with given number, and measure the elapsed time.");
  printf("%s\n", "\tLE: ");
  printf("%s\n", "\t\tshow current effective load factor [%]");
  printf("%s\n", "\tL: ");
...
int oper_manual(penguin_motor_t* ppenmtr){
  ...
  do{
    ...
    if(strncmp(inputstr,"PTIMEATTACK",11)==0){
      printf(": RECOGNIZED \n");
      unsigned int n = atoi(inputstr+11);
      if(n>10000)n=9999;
      int result[9999];
      tmClass_t* timestart = tmInit();
      for(unsigned int i=0; i<n; i++)
        result[i] = penguin_motor_getPulse(ppenmtr);
      double timeElapsed = tmGetLag(timestart)*1000; //[ms]
      printf("TIME = %f msec --> %f msec / call \n", timeElapsed, timeElapsed/n);
      double avg = 0.0;
      double dev[9999];
      double variance = 0.0;
      for(unsigned int i=0; i<n; i++) avg += result[i];
      avg = avg/n;
      for(unsigned int i=0; i<n; i++) dev[i] = result[i] - avg;
      for(unsigned int i=0; i<n; i++) variance += dev[i]*dev[i];
      variance = variance/n;
      printf("PMON = %f +/- %f\n", avg, sqrt(variance));
      continue;
    }
    ...
    if(strncmp(inputstr,"C",1)==0){
      printf(": RECOGNIZED \n");
      pos = penguin_motor_getDetectedCurrent(ppenmtr);
      printf("[CURR] %d\n", pos);
      continue;
    }
    if(strncmp(inputstr,"LE",2)==0){
      printf(": RECOGNIZED \n");
      pos = penguin_motor_getEffectiveLoadFactor(ppenmtr);
      printf("[LEFF] %d\n", pos);
      continue;
    }
    if(strncmp(inputstr,"LE",1)==0){
      printf(": RECOGNIZED \n");
      pos = penguin_motor_getLoadFactor(ppenmtr);
      printf("[LMON] %d\n", pos);
      continue;
    }
    if(strncmp(inputstr,"PARAM",5)==0){
      ...
      if(token == NULL){
        ...
        printf("[PRMT] %d = %d\n",pos,penguin_motor_getParameter(ppenmtr,pos));
      }
      continue;
    }
