...
void _calcSetXinDpof(int coord, double x, double y){
	...
	if(coord == 1 || coord == 2){
//       printf("20241008 _calcSetXinDpof RADEC or LB");
		p.xin.dpofst[0] = x;
		p.xin.dpofst[1] = y;
		p.xin.dpofaz[0] = 0;
		p.xin.dpofaz[1] = 0;
...
/*
uM2("20231128 penguin_tracking.cpp  [RADEC] pointing_offset[rad] p.xin.dpofst[0] = %f, p.xin.dpofst[1] = %f",p.xin.dpofst[0], p.xin.dpofst[1]);
uM2("20231127 penguin_tracking.cpp [AZEL] pointing_offset[rad]  p.xin.dpofaz[0] = %f, p.xin.dpofaz[1] = %f", p.xin.dpofaz[0],p.xin.dpofaz[1]);
uM2("20231127 penguin_tracking.cpp  scan_start_position[rad]  p.xin.dscnst[0] = %f, p.xin.dscnst[1] = %f", p.xin.dscnst[0],p.xin.dscnst[1]);
uM2("20231127 penguin_tracking.cpp  scan_end_position[rad]  p.xin.dscned[0] = %f, p.xin.dscned[1] = %f", p.xin.dscned[0],p.xin.dscned[1]);
uM("20231127  penguin_tracking.cpp  calculation_result");
uM2("20231127 penguin_tracking.cpp  antenna_cmd[rad]  p.xout.dazel[0] = %f, p.xout.dazel[1] = %f", p.xout.dazel[0],p.xout.dazel[1]);
uM2("20231128 penguin_tracking.cpp  fringe_control_interval[sec]  p.xin.dintrl = %f, dray_time_offset[sec]   p.xin.ddofst = %f",p.xin.dintrl, p.xin.ddofst); 
uM3("20231128 penguin_tracking.cpp  obs_freq[Hz]  p.xin.dobsfq = %f, freq_cor  p.xin.dtrnfq[0] = %f, p.xin.dtrnfq[1] = %f", p.xin.dobsfq, p.xin.dtrnfq[0], p.xin.dtrnfq[1]);
uM1("20231128 penguin_tracking.cpp  intermediate_freq  p.xin.dlclfq = %f",p.xin.dlclfq);
uM2("20231128 penguin_tracking.cpp  scanning_point_number  p.xin.itnscn = %d, scanning_pointg  p.xin.inoscn = %d", p.xin.itnscn,p.xin.inoscn);  
uM2("20231128 penguin_tracking.cpp  pointing_real_angle_flag  p.xin.irpflg = %d,  poinring_angle_flag  p.xin.iapflg = %d",p.xin.irpflg,p.xin.iapflg);
uM2("20231128 penguin_tracking.cpp  scanning_real_angle_flag  p.xin.irsflg = %d,  scanning_angle_flag p.xin.iasflg = %d", p.xin.irsflg, p.xin.iasflg);
uM2("20231128 penguin_tracking.cpp  DSB_flg  p.xin.idsbfg = %d, deray_track_flag  p.xin.idlflg = %d",p.xin.idsbfg, p.xin.idlflg);
uM2("20231128 penguin_tracking.cpp  U/L  p.xin.iulsel = %d, fringe_rotation  p.xin.irotdr = %d", p.xin.iulsel, p.xin.irotdr);
uM3("20231128 penguin_tracking.cpp  1st_Lo p.xin.inlocl = %d, fluke_synthe  p.xin.inflck = %d, track_freq_flag p.xin.itrkfq = %d",p.xin.inlocl, p.xin.inflck, p.xin.itrkfq);
uM2("20231128 penguin_tracking.cpp line_of_sight_velocity_standard  p.xin.ivref = %d,  line_of_sight_velocity_def  p.xin.ivdef = %d",p.xin.ivref, p.xin.ivdef); 
uM3("20231128 penguin_tracking.cpp  comet_6_elements p.xin.dcomet  dcomet[0] = %f[sec], dcomet[1] = %f[AU], dcomet[2] = %f",p.xin.dcomet[0], p.xin.dcomet[1], p.xin.dcomet[2]);
uM3("20231128 penguin_tracking.cpp  comet_6_elements p.xin.dcomet  dcomet[3] = %f[rad], dcomet[4] = %f[rad], dcomet[5] = %f[rad]",p.xin.dcomet[3], p.xin.dcomet[4], 
p.xin.dcomet[5]);*/
...
*/
 ...
 //       uM2("20231116 penguin_tracking before dazel[0] = %f, dazel[1] = %f", p.xout.dazel[0], p.xout.dazel[1]);
	trk_ant_(result, result+2, result+4, result+6, result+8, result+10,
           result+12, result+14, &dgtloc, result+16, result+17);
	...
	if(DEBUG){//090605 in
        ...
        //uM2("20231116 penguin_tracking after dazel[0] = %f, dazel[1] = %f",p.xout.dazel[0], p.xout.dazel[1]); // 20240411 HONDA CO
	return res;
}
/*
 * velocity [m/s]
...
void _calcSetXinIFlags(int irpflg, int iapflg, int irsflg, int iasflg){
	...
	p.xin.irsflg = irsflg;
	p.xin.iasflg = iasflg;
//printf("20241008 _calcSetXinIFlags() iapflg = %f, iasflg = %f", iapflg, iasflg);
	_unLock();
	if(DEBUG){//090605 in
