#ifndef INCLUDE_GUARD_UUID_b00ad4de_5d40_482f_b6e9_6047608f9f8c
#define INCLUDE_GUARD_UUID_b00ad4de_5d40_482f_b6e9_6047608f9f8c
/*!
  ¥file be.cpp
  ¥author R.Enohi
  ¥date 2024.11.26
...
int beReqInit(){
  ...
  uM1("beReqInit() p.BeNum: %d", p.BeNum); //20240813
  uM1("beReqInit(); p.ans[0].endExeFlag: %d", p.ans[0].endExeFlag); //20240814
 ...
 //   printf("saaacqInit(); size=%d paramSize=%d¥n", size, initDat->paramSize);  //20240814
    if(ret <= 0){
    ...
    for(int j = 0; j < 5; j++){
      ...
      if(ret <= 0){
	uM2("beReqInit(); netclRead(BeIp%02d); ret=%d error!!", i + 1, ret);  //20240814
	err = 2;
	break;
      }
      if(j == 0){
	getSize = p.ans[i].size;
//	printf("p.ans.size=%d¥n",p.ans[i].size);  //20240814
      }
      size += ret;
  //    printf("debug size=%d¥n", size); //20240814
      if(size >= getSize){
	err = 0;
	break;
      }
    }
//    uM1("debug beReqInit(); netclRead(); getSize=%d", getSize);  //20240814

     //20240813
/*    uM1("p.ans_size: %d", sizeof(p.ans[i]))
      uM2("p.ans[%d].size: %ld", i, p.ans[i].size);
      uM2("beReqInit(); p.ans[%d].endExeFlag: %d", i, p.ans[i].endExeFlag);
      uM2("p.ans[%d].acqErr: %d", i, p.ans[i].acqErr);
      uM2("p.ans[%d].tsys: %f", i, p.ans[i].tsys);
      uM2("p.ans[%d].totalPower: %f", i, p.ans[i].totalPower);
      uM2("p.ans[%d].nbrOfChannel: %d", i, p.ans[i].nbrOfChannel);
      uM2("p.ans[%d].spectrum: %lf", i, p.ans[i].spectrum);
    */
    if(err == 2){
...
/*! ¥fn int beReqStart(int OnOffRoad, int returnFlag, unsigned long startTime,
  ...
  ¥brief 角度の単位は全て[rad]
  ¥param[in] OnOffRoad 観測点の種類を設定する。 0:OnPoint(PSW) 1:OffPoint 2:RSky 3:OnPoint(OTF) //20240810
  ...
  ¥param[in] FQTRK CO静止周波数[GHz] //20240810
  ¥param[in] FQTRK2 CI静止周波数[GHz] //20240810
  ¥param[in] FQTRK3 CO静止周波数[GHz] 2系統目//20241126
  ¥param[in] FQTRK4 CI静止周波数[GHz] 2系統目//20241126
  ¥param[in] FQIF1 1stIFのローカル周波数[GHz]
  ¥param[in] VRAD 観測系からの視速度[km/s]
  ¥param[in] IFATT IFのステップアッテネーターの値(ONOFFの時)[dB]
  ¥param[in] IFATT2 IFのステップアッテネーターの値(ONOFFの時)[dB] //20240810
  ¥param[in] IFATT3 IFのステップアッテネーターの値(ONOFFの時) 2系統目[dB]  //20241126
  ¥param[in] IFATT4 IFのステップアッテネーターの値(ONOFFの時) 2系統目[dB] //20241126
  ¥param[in] LO2_1 2nd Local CO[GHz] //20240810
  ¥param[in] LO2_2 2nd Local CI[GHz] //20240810
  ¥param[in] LO2_3 2nd Local CO[GHz] 2系統目//20241126
  ¥param[in] LO2_4 2nd Local CI[GHz] 2系統目//20241126
  ¥param[in] weath 気象データ。[気温][気圧][水上気圧]の順
  ¥param[in] wind 気象データ(風)。[風向][風速][最大瞬間風速]の順
  ¥param[in] TAU 大気の厚さ(.deviceファイルで指定)
...
int beReqStart(int OnOffRoad, int returnFlag, unsigned long startTime,
               ...
               double* DRADEC, double* DLB, double* DAZEL,
               double* PAZEL, double* RAZEL,
               double PA, double FQTRK, double FQTRK2, double FQTRK3, double FQTRK4,
               double FQIF1, double VRAD, double IFATT, double IFATT2, double IFATT3, double IFATT4,
  ...
  for(int i = 0; i < p.BeNum; i++){
    ...
    p.req[i].OnOffRoad = OnOffRoad;        //20240810 tkb32Func.cppのOnOffRの値にしたい
    ...
    p.req[i].FQTRK = FQTRK;  //20240810
    p.req[i].FQTRK2 = FQTRK2;  //20240810
    p.req[i].FQTRK3 = FQTRK3;  //20241126
    p.req[i].FQTRK4 = FQTRK4;  //20241126
    p.req[i].FQIF1 = FQIF1;
    p.req[i].VRAD = VRAD;
    p.req[i].IFATT = (int)IFATT;
    p.req[i].IFATT2 = (int)IFATT2;  //20240810
    p.req[i].IFATT3 = (int)IFATT3;    //20241126
    p.req[i].IFATT4 = (int)IFATT4;  //20241126
    p.req[i].LO2_1 = LO2_1;  //20240810
    p.req[i].LO2_2 = LO2_2;  //20240810
    p.req[i].LO2_3 = LO2_3;  //20241126
    p.req[i].LO2_4 = LO2_4;  //20241126
    ...
    p.req[i].TAU = TAU;                      //!< ant30_phaseC0.confから読み取る  //20240905
   // p.req[i].BATM = BATM;
    p.req[i].BATM = weath[0]; //20241126 気象装置から取得した気温をここへ
   // p.req[i].IPINT = IPINT;
    p.req[i].IPINT = 270.0; //20240918
//uM3("beReqStart(); TAU =%f,  BATM = %f, IPINT = %f",p.req[i].TAU,p.req[i].BATM, p.req[i].IPINT);

    ret = _myWrite(i, (const unsigned char*)&p.req[i], p.req[i].size);
    //uM1("beReqStart(); netclWrite(); ret=%d", ret);
    if(ret <= 0 || ret != p.req[i].size){
      //uM3("beReqStart(); _myWrite(BeIp%02d); [%d(%s)] error!!", i + 1, ret, errnoStrAnt30(ret));
      const char* hoge = "ERR";
      uM3("beReqStart(); _myWrite(BeIp%02d); [%d(%s)] error!!", i + 1, ret, hoge);//errnoStrAnt30(ret));
      if(ret <= 0){
	e = BE_NET_ERR;
      }
      else{
    ...
    for(int j = 0; j < 5; j++){
     ...
     // uM1("20240826 _myRead(); expected buffer size = %ld bytes", buf + size);
     // uM1("20240826 _myRead(); buffer size = %ld bytes", getSize - size);
      if(ret <= 0){
        const char* hoge = "ERR";
	uM3("beReqStart(); _myRead(BeIp%02d); [%d(%s)]", i + 1, ret, hoge);//errnoStrAnt30(ret));
	err = 2;
	break;
	//continue;
	//return ret;
...
// printf("20231106 be.cpp beGetAns start!");
//  uM1("20231106 be.cpp p.BeUse[beId] = %d",p.BeUse[beId]);
  printf("begetans0\n");
  if(init == 0){
...
//uM1("20231106 be.cpp p.BeUse[beId] = %d",p.BeUse[beId]);
//uM3("20231101; be.cpp init = %d, p.BeUse = %d, beId = %d", init, p.BeUse, beId);
  if(p.BeUse[beId] != 1){
