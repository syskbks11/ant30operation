#ifndef INCLUDE_GUARD_UUID_798af54b_8b2d_467d_b78f_a2c29fb6d1a0
#define INCLUDE_GUARD_UUID_798af54b_8b2d_467d_b78f_a2c29fb6d1a0
/*!
  ¥file trk.cpp
  ¥author Y.Koide
  ¥date 2006.11.20
...
FILE *fp; // for azel logfile // 20240409 HONDA
...
int trkInit(){
  ...
  */
  ...
  if(controlBoardGetAntenna() == CONTROL_BOARD_USE_FULL){//090611 in
    ...
    */
    ret = acuInit();
    if(ret){
      uM1("trkInit(); acuInit(); ret = %d error!!",ret);
      return ret;
    }
    ret = acuGetACU();
    if(ret){
  ...
  */
  ...
  //! logfile creation //20240409 HONDA
  char tmp[128];
  time_t t;
  struct tm* tmt;
  time(&t);
  tmt = localtime(&t);
  sprintf(tmp, "../../tcs01/log/ant30/azel-%04d%02d%02d%02d%02d%02d.log",
          tmt->tm_year+1900,tmt->tm_mon+1, tmt->tm_mday, tmt->tm_hour, tmt->tm_min, tmt->tm_sec); //20240409 HONDA
  fp = fopen(tmp,"a");
  //! --------------- !//
...
int trkInitParam(){
  ...
  */
  ...
  if(ret){
    const char* hoge = "ERR";//errnoStrAnt30(ret);
    uM2("trkInitParam(); calcTrkCal(); [%d(%s)] error!!", ret, hoge);
    p.thrdRet = ret;
    return p.thrdRet;
  }
  _trkCalZoneTypeInit();
...
void* _trkThrdMain(void* _p){
  ...
  //! Main Loop
  // variable definitions // 20240409 HONDA
  int rastFlg = 0;
  double sof[2];
  double offsetMSec;
  int tmp_loadstatus[6]; // temporal use for loadfactor debug
  int is_show_message = 1; //for debug
  double dtime;
  /////////////////////////
  p.thrdRun = 1;
  while(p.thrdRun){
    ...
    if(!p.rastStart){
      ...
      //   uM2("trk timing check: %f sec (should be < %f sec)",tmGetLag(p.vtm),p.TrkInterval);
      //   is_show_message = 0;
      // }
      // <--
      dtime = tmGetLag(p.vtm);
      if(is_show_message==1 && dtime>p.TrkInterval){
        uM2("too much trk calc time = %f sec (should be < %f sec)",tmGetLag(p.vtm),p.TrkInterval);
        is_show_message = 0;
      }
      else{
        is_show_message = 0;
    ...
    is_show_message = 1; // for debug
    // uM1("20231102; trk.cpp p.thrdRun = %d", p.thrdRun);
    //uM("_trkThrdMain()...");
    //raster scanのスキャン速度設定 20240202
    // double azel[0] = 0.2; //[deg/sec]
    // double azel[1] = 0.2;

    // double v_az = acuSetAcuVeloc(azel[0]);
    // double v_el = aciSetAcuVeloc(azel[1]); 
 
    // uM2("2024 penguin_motor_setVelocity; AZ: %f, EL: %f", v_az, v_el);

    //! RastStartフラグが立っていたらRaster処理を行う。
    //double a = tmGetDiff(p.rastStartTime, 0);
    //printf("_trkThrdMain(); debug tmGetDiff(); diff=%lf sec¥n", a);
    //int rastFlg = 0; // 20240409 HONDA CO
    rastFlg = 0;
    if(p.rastStart){
      if(p.rastSofNo > 0){
        rastFlg = 1;
    ...
    //uM2("20231102; trk.cpp p.rastStart = %d, rastFlg = %d", p.rastStart, rastFlg); 
    if(rastFlg){
      //! ラスター中
      g_rastEndFlag = RUN_RASTENDF;
      //double sof[2];//20240409 HONDA CO
      if(p.rastStart == 1){//update080529*1
        if(p.rastSofNo < p.rastSofSize){
          ...
          */
          //! 1msec以内になるまで待つ。
	  //double offsetMSec = p.rastSofNo * p.TrkInterval * 1000.0; //20240409 HONDA CO
          offsetMSec = p.rastSofNo * p.TrkInterval * 1000.0;
          dtime = tmGetDiff(p.rastStartTime,offsetMSec);
          if(dtime<0)
            uM2("too much trk calc time in raster (#%d) : %f sec delayed",p.rastSofNo,-1*dtime);
	  // uM2("rasterscan timing check: T_now - T_rastScanStart(#%d) = %f sec (waiting until <= 0.001)",
          //     p.rastSofNo, tmGetDiff(p.rastStartTime,offsetMSec));
          while(tmGetDiff(p.rastStartTime, offsetMSec) > 0.001){
	    //uM1("raster scan timing check = %f sec (waiting until <= 0.001)", tmGetDiff(p.rastStartTime,offsetMSec));
            //tmSleepMSec(0.001 * 10.0); // 20240209 added HONDA instead of below lines //20240409 HONDA CO
            //if(controlBoardGetAntenna() == CONTROL_BOARD_USE_FULL){//090629 for 30 cm // 20240209 comment-out HONDA for speedup
            //  ret = acuGetACU();
            //  if(ret){
          ...
          /*acuGetRAZEL(acuStatDazel);
            uM2("_trkThrdMain; real(AZ,EL) = (%f,%f)", acuStatDazel[0], acuStatDazel[1]);*/
          p.rastSofNo++;
    ...
    ret = 0;
    if(controlBoardGetAntenna() == CONTROL_BOARD_USE_FULL){//090619 for 30 cm
      ...
      */
      ...
      // 20240409 HONDA CO
      // double myacuazel[2];
      // acuGetRAZEL(myacuazel);
      // const double mydoaz2=calcGetDoAz2();
      // const double mydoel2=calcGetDoEl2();
      // uM4("cmd = %f, %f <--> data = %f, %f",mydoaz2,mydoel2,myacuazel[0],myacuazel[1]);
    }

    //! 追尾状態の算出
    _trkCalTrackStatus(ret);

    /*** 2011/01/26 in ***/ // --> modified by HONDA 20240409
    // actual AZEL
    acuGetRAZEL(acuStatDazel);
    // command AZEL
    doaz2 = calcGetDoAz2();
   ...
   //  uM4("AZcmd, AZreal, ELcmd, ELreal, %f, %f, %f, %f",
   //      doaz2, acuStatDazel[0], doel2, acuStatDazel[1]);
    fprintf(fp, "[%s] AZcmd, AZreal, ELcmd, ELreal, %f, %f, %f, %f\n",
            uGetDate(), doaz2, acuStatDazel[0], doel2, acuStatDazel[1]);
    fflush(fp); //20240813

    // debug load factor 20240409 HONDA
   //  acuGetACULoadStatus(tmp_loadstatus);
   //  fprintf(fp, "[%s] AZreal, ELreal, AZcmd, ELcmd, cur_el, effload_el, load_el, %f, %f, %f, %f, %d, %d, %d\n",
   //          uGetDate(), acuStatDazel[0], acuStatDazel[1], doaz2, doel2,
   //          tmp_loadstatus[0],tmp_loadstatus[1],tmp_loadstatus[2]);
   // fflush(fp);

    // AZ, EL + effload_el 20240430 enohi
    // acuGetACULoadStatus(tmp_loadstatus);
    // fprintf(fp, "[%s] AZreal, ELreal, AZcmd, ELcmd, effload_el,  %f, %f, %f, %f, %d\n",
            // uGetDate(), acuStatDazel[0], acuStatDazel[1], doaz2, doel2,tmp_loadstatus[1]);
    //fflush(fp);

    // AZ, EL + effload_az, effload_az  20240806 enohi
    // acuGetACULoadStatus(tmp_loadstatus);
    // fprintf(fp, "[%s] AZreal, ELreal, AZcmd, ELcmd, effload_az, effload_el,  %f, %f, %f, %f, %d, %d\n",
    //         uGetDate(), acuStatDazel[0], acuStatDazel[1], doaz2, doel2, tmp_loadstatus[0],tmp_loadstatus[1]);
  // fflush(fp);
    ...
    /* 090605 out, since penguin_tracking is now thread-safe.
       uLock();
    */
    //	struct rusage t; getrusage(RUSAGE_SELF, &t);//090619 in to measure performance. 
    //	double start = t.ru_utime.tv_sec*1000 + t.ru_utime.tv_usec/1000;//[ms]
    ...
    */
    ...
    //	double end = t.ru_utime.tv_sec*1000 + t.ru_utime.tv_usec/1000;//[ms]
    //	double timeElapsed = (end-start);//[ms]
    //double timeElapsed = tmGetLag(timeStart)*1000;//[ms]
    //uM1("calcTrkCal() used %lf ms.", timeElapsed);
    ...
    //_trkCalZoneType(calcGetDoAz2());//081117 in//20240409 HONDA CO
    _trkCalZoneType(doaz2);
    ...
    err=0;
    //printf("acuSetDoAzC: %f, acuSetDoElC: %f \n",doaz2,doel2); // 20240208 コメントアウト（本多）
    acuSetDoAzC(doaz2);
    acuSetDoElC(doel2);
    ...
    /*** 2011/01/26 in ***/
    //uM2("_trkThrdMain(); cmnd(AZ,EL) = (%f,%f)", doaz2, doel2);//debug
    ...
    if(doel2 < p.dazelmin[1] || doel2 > p.dazelmax[1]){
      ...
      if(err){
        ...
        if(doel2 > p.dazelmax[1]){
          printf("El_limit acuSetDoElC: %f (%f)\n", p.dazelmax[1] - marg, p.dazelmax[1]);
  ...
  if(controlBoardGetAntenna() == CONTROL_BOARD_USE_FULL){//090611 in
    ...
    */
    ...
    if(!ret){
      ...
      //	double acuStatDazel[2];
      acuGetRAZEL(acuStatDazel);
      printf("Stop acuSetDoAzC: %f, acuSetDoElC: %f\n",acuStatDazel[0],acuStatDazel[1]);
  ...
  while(p.thrdStatusRun){
    ...
    if(controlBoardGetAntenna() == CONTROL_BOARD_USE_FULL){//090611 in
      ...
      */
      ...
      if(ret){
        const char* hoge = "ERR";//errnoStrAnt30(ret);
        uM2("_trkThrdStatus(); acuGetACU(); [%d(%s)]", ret, hoge);
	p.thrdRet = ret;
      }
      ret = acuSafetyCheck();
    }
...
int trkEnd(){
  ...
  */
  ...
  //! logfile closing //20240409 HONDA
  fclose(fp);
...
void _trkCalTrackStatus(const int reset){
  ...
  */
  ...
  // 20240208 コメントアウト（本多）下２行
  //uM3("20231124  p.deazel[0] = %f, acuStatDazel[0] = %f, doaz2 = %f", p.deazel[0], acuStatDazel[0], doaz2);
  //uM3("20231124  p.deazel[1] = %f, acuStatDazel[1] = %f, doel2 = %f", p.deazel[1], acuStatDazel[1], doel2);
  ...
  // uM1("20231124 mikake Err Az p.deazel[0] = %f ",p.deazel[0]);  
  //  uM2("20231115 (dAz, dEl)=(%.2lf\",%.2lf\")", p.deazel[0]*3600, p.deazel[1]*3600);
  ...
  //  uM2("20231115 dAz = %f, p.AntAzElMarg[0] = %f", dAz, p.AntAzElMarg[0]);
  //  uM2("20231115 dEl = %f, p.AntAzElMarg[1] = %f", dEl, p.AntAzElMarg[1]);

  //uM1("stat = %d", stat);//20240208 コメントアウト（本多）
  ...
  if(stat == 0){
    ...
    if(p.trackStatCnt > p.AntAzElMargThrsd){
      ...
      p.trackStatCnt++;
      p.trackStat = 1;
      //uM1("20240205 p.trackStatCnt= %d",p.trackStatCnt);
    }
  }
  else{
    //! Swing
    p.trackStatCnt = 0;
    p.trackStat = stat;
    //uM1("20240205 swing p.trackStat = %d", p.trackStat);//20240208 コメントアウト（本多）
  }
  ...
  if(p.curr.sofNo >= scanTableGetNumberOfPoints()){//081117 in
    ...
    *id = 0;
    ...
    if(mapNo == 0){
      ...
      // uM1("20231115 trk.cpp dof = %f",sof);
	...
	sy =(- onX * b + onY * a) * sec2rad;
        // uM4("20231115 trk.cpp a = %f, b = %f, sx = %f, sy = %f", a, b, sx, sy);   
        /* 081117 out
	...
	for(int i = 0; i < p.rastSofSize; i++){
	  ...
	  p.rastSof[i*2+1] = sy + dy * (double)i / (double)(p.rastSofSize-1);
          // uM1("20231108; trk.cpp p.rastSofSize = %d", p.rastSofSize);
    ...
    // uM2("20231115 debug trkSetSofNext(); sofNo[%d] mapNo[%d]", p.curr.sofNo, mapNo);
  }
...
void setParam(){
  //int i;
  //char tmp[16];
  p.dazelmax[0] = 350; //20231124 (350) 20240209(265.43)
  p.dazelmin[0] = -263.62; //20231124 (10)
  p.dazelmax[1] = 77.51; //20231124 (88)
  p.dazelmin[1] = 2.11; //20231124 (5)
  p.AntAzElMarg[0] = 0.005; //20240126 (0.1)
  p.AntAzElMarg[1] = 0.005; //20240126 (0.1)
 ...
 //ant30.conf 20240814
  if(confSetKey("AntPosLo"))
    p.AntPosLo = atof(confGetVal());
  if(confSetKey("AntPosLa"))
    p.AntPosLa = atof(confGetVal());
  if(confSetKey("AntAltitude"))
    p.AntAltitude = atof(confGetVal());
