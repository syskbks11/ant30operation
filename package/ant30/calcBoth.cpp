#ifndef INCLUDE_GUARD_UUID_d1c98f9d_71f7_4fcd_b669_ef9f31558f50
#define INCLUDE_GUARD_UUID_d1c98f9d_71f7_4fcd_b669_ef9f31558f50
/*!
  ¥file calcBoth.cpp
  ¥author NAGAI Makoto
  ¥date 2008.11.14
...
//FILE *fp; //20240130
//static char fname[256]; //20240130
...
/*
  ...
  * thread 1, phase A0
  */
/*20240106 lintkb/src/u/u.cppから転用　ログファイルネームの設定*/
/*int uInit(const char* projectName){
 ...
 time(&t); 
 tmt = localtime(&t);
 sprintf(fname, "%s-%04d%02d%02d%02d%02d%02d.log", projectName, tmt->tm_year+1900,tmt->tm_mon+1, 
...
int calcInit(){
/*
printf("2024 calcBoth.cpp calcInit\n");
 time_t t;
 struct tm* tmt;   
 const char* projectName = "AZEL_new";

 time(&t); 
 tmt = localtime(&t);
 sprintf(fname, "../tcs01/log/ant30/%s-%04d%02d%02d%02d%02d%02d.log", projectName, 
tmt->tm_year+1900,tmt->tm_mon+1, 
tmt->tm_mday, tmt->tm_hour, tmt->tm_min, tmt->tm_sec);
fp = fopen(fname,"a"); //20240130
*/
  //p = param;
  /* 090604 out
...
int calcTrkCal(int currOffFlag, int currOffNo){
  //printf("2024 calcBoth.cpp calcTrkCal\n"); //20240409 HONDA CO
  /* 090604 out
     char cerr[256];
  ...
  */
  ...
  diazel[0] = azel[0];
  diazel[1] = azel[1];
//uM2("20231116 calcBoth diazel[0] = %f, diazel[1] = %f", diazel[0], diazel[1]);
//uM2("20231116 calcBoth azel[0] = %f, azel[1] = %f", azel[0], azel[1]);
  //! trk_30 (大気差補正)
...
// uM2("20231117  trk_30 diazel[0] = %lf, diazel[1] = %lf", diazel[0], diazel[1]);
  if(ival != 0){
...
//uM2("20240119 calcBoth trk_30 pCalc.doazel[0] = %f, pCalc.doazel[1] = %f", pCalc.doazel[0], pCalc.doazel[1]);
//uM2("20231124 calcBoth pCalc.doazel2[0] = %f, pCalc.doazel2[1] = %f", pCalc.doazel2[0], pCalc.doazel2[1]); //20240208 コメントアウト（本多）
// fprintf(fp, "[%s] AZ_real, EL_real, AZ_cmd, EL_cmd, %f, %f, %f, %f\n",uGetDate(), pCalc.doazel[0], pCalc.doazel[1],pCalc.doazel2[0], pCalc.doazel2[1]); 
//20240130	
  ...
  */
//  uM2("20240118 calcBoth.cpp daz = %f, del = %f", daz, del);
  //uM2("20240118 calcBoth pCalc.doazel2[0] = %f, pCalc.doazel2[1] = %f", pCalc.doazel2[0], pCalc.doazel2[1]); // 20240411 HONDA CO
  pCalc.doazel2[0] += daz;
  pCalc.doazel2[1] += del;
...
//uM4("20231109; calcBoth.cpp pCalc.doazel2[0] = %f, daz = %f, pCalc.doazel2[1] = %f, del = %f",pCalc.doazel2[0], daz, pCalc.doazel2[1], del);
...
if(scanFlag == SCANTABLE_FLAG_CROSS){
  uM4("(Az, El) = (%lf, %lf), scan offset = (%lf, %lf)", pCalc.doazel2[0], pCalc.doazel2[1], glacier_ScanOffsetGetX(pCalc.scanOffset) * rad2sec, glacier_ScanOffsetGetY(pCalc.scanOffset) * rad2sec); // 20240411 HONDA CO
}
// fprintf(fp, "[%s] AZ_real, EL_real, %f, %f\n",uGetDate(), pCalc.doazel2[0], pCalc.doazel2[1]); 
//20240130   
  //timeElapsed = tmGetLag(timeStart)*1000;//[ms]
...
//  uM1("20231108; calc.Both.cpp calcGetPAZEL() PAZEL = %f", PAZEL);
  return;
}
...
//  uM1("20231116; calc.Both.cpp calcGetDoAz2()  pCalc.doazel2[0] = %lf", pCalc.doazel2[0]);
...
// uM1("20240118; calcBoth.cpp calcGetDoEl2()  pCalc.doazel2[1] = %lf", pCalc.doazel2[1]);
  return pCalc.doazel2[1];
}

double calcGetMapCenterDec(){
//  uM1("20231108; calc.Both.cpp calcGetMapCenterDec()  pCalc.dgtdat[1] = %lf",pCalc.doazel[1][1]);
  return pCalc.dgtdat[1][1];
}
double calcGetAntennaLat(){
//  uM1("20231108; calc.Both.cpp calcGetMapCenterDec()  pCalc.dgtdat[1] = %lf", pCalc.doazel[0][1]);
...
void calcGetDAZEL(double* DAZEL){
  ...
  // uM2("20231115 calcBoth.cpp DAZEL[0] = %lf  DAZEL[1] = %lf", DAZEL[0], DAZEL[1]);
  }else{
...
/*! ¥fn void calcGetScanOff(int* coord, double* sof)
  ...
  ¥param[out] coord 1:RADEC 2:LB 3:AsZEL
  ¥param[out] sof スキャンオフセットの値 [sec]
*/
void calcGetScanOff(int* coord, double* sof){
  *coord = glacier_ScanOffsetGetCoordinate(pCalc.scanOffset);
//090529 in
  /* 090529 out
...
//  uM2("20231115 calcBoth.cpp CalcGetScanOff() sof[0] = %.2lf  sof[1] = %.2lf", sof[0], sof[1]);
  /* 090529 out
...
/*! ¥fn void setParam()
  ...
  */
void setParam(){
//printf("2024 calcBoth.cpp setparam\n");
...
// 20240814 今回トラッキングをかけるのは1st Local
  if(confSetKey("Lo1"))
