#ifndef INCLUDE_GUARD_UUID_119ffa2a_2222_441d_aa6c_06c4bf907561
#define INCLUDE_GUARD_UUID_119ffa2a_2222_441d_aa6c_06c4bf907561
/*!
...
int main(int argc, char* argv[]){
  ...
  */
  ...
  memset(&p, 0, sizeof(p));
  sprintf(tmp, "%s/ant30/ant30", logPath);//090813 in
  /* 090813 out
     sprintf(tmp, "%s/ant30", logPath);
  */
  uInit(tmp);
  uM("Starting ant30");
  ...
  */
  ...
  while(act){
    ...
    //phase A1: work with requests from tkb32 cliant.
    while(act){
      //printf("debug start netsvRead()...\n");
      ret = netsvRead(p.net, (unsigned char*)&size, sizeof(size));//081125 in
      //      ret = netsvRead(p.net, (unsigned char*)&size, 4);//081125 out
      if(ret < 0){
        uM1("MAIN__(); netsvRead(); ret %d", ret);
        break;
      }
      else if(ret != sizeof(size)){//081125 in
        //      else if(ret != 4){//081125 out
        //! タイムアウトか転送データ不足
        continue;
      }
      reqSize = size;
      sizeCnt = sizeof(size);//081125 in
      //      sizeCnt = 4;//081125 out
      //uM1("debug MAIN__(); reqSize = %d", reqSize);
      req = new unsigned char[reqSize];
      memcpy(req, &size, sizeof(size));//081125 in
      //      memcpy(req, &size, 4);//081125 out
