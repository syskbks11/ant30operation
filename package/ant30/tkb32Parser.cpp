#ifndef INCLUDE_GUARD_UUID_f9de4c62_f95f_4515_a3ba_fc95885c5a64
#define INCLUDE_GUARD_UUID_f9de4c62_f95f_4515_a3ba_fc95885c5a64
/*!
  ...
  case 2:
    ...
    */
    ...
    /* 081208 out
       ...
       if(ret){
       const char* hoge = "ERR";
       uM2("setReqData(); chopRepeat(); [%d(%s)] error!!", ret, hoge);//errnoStrAnt30(ret));
       (*p).reqRet = ret;
       }
       }
       ...
       if(ret){
       const char* hoge = "ERR";
       uM2("setReqData(); chopRepeat(); [%d(%s)]", ret, hoge);//errnoStrAnt30(ret));
       (*p).reqRet = ret;	
       }
       }
    ...
    */
    ...
    /* 081208 out
       ...
       if(ret){
       const char* hoge = "ERR";
       uM2("setReqData(); sacqReqState(); [%d(%s)] error!!", ret, hoge);//errnoStrAnt30(ret));
       (*p).reqRet = 1;
       }
       else{
       //int exe;
       ...
       if(ret){
       const char* hoge = "ERR";
       uM2("setReqData(); integStart(); [%d(%s)] error!!", ret, hoge);//errnoStrAnt30(ret));
       (*p).reqRet = ret;	  
       }
       //}
       }//!< if(beReqState())
...
int getAnsData(unsigned char** ans, int *ansSize){
  ...
  //uM1("20231030; ansflg = %d", pParser.ans->ansFlg);
  //uM2("20231030; ansSize = %d, size = %ld", ansSize, pParser.ans->size);
  ...
  *ansSize = size;
  uM1("20231030; ansflg = %d", pParser.ans->ansFlg);
  uM2("20231030; ansSize = %ld, size = %ld", *ansSize, size);
  return 0;
}
