#ifndef INCLUDE_GUARD_UUID_758b662c_989d_49b9_9f97_a50b24cbef73
#define INCLUDE_GUARD_UUID_758b662c_989d_49b9_9f97_a50b24cbef73
/*!
¥file scanTable.cpp
¥author NAGAI Makoto
¥date 2008.11.15
...
int scanTableInit(){
  ...
  if(confSetKey("OffMode"))
	...
	if(!((scan.ScanFlag >= SCANTABLE_FLAG_ONOFF && scan.ScanFlag <= SCANTABLE_FLAG_CROSS)
	 || (scan.ScanFlag >= SCANTABLE_FLAG_SOURCE && scan.ScanFlag <= SCANTABLE_FLAG_LEE))){//090611 in
/* 090611 out
    if(!((scan.ScanFlag >= 1 && scan.ScanFlag <= 6) || (scan.ScanFlag >= 1001 && scan.ScanFlag <= 1002))){
*/
      uM1("ScanFlag(%d) error!!", scan.ScanFlag);
      return -1;
    }

	if(scan.ScanFlag >= SCANTABLE_FLAG_ONOFF && scan.ScanFlag <= SCANTABLE_FLAG_CROSS){//090611 in
/* 090611 out
...
*/
    ...
    else if(scan.ScanFlag == SCANTABLE_FLAG_SOURCE){//090611 in
/* 090611 out
...
*/
  ...
  //! Initialize map pattern
	if(scan.ScanFlag >= SCANTABLE_FLAG_ONOFF && scan.ScanFlag <= SCANTABLE_FLAG_CROSS){//090611 in
		//! On,5Point,Grid,Random,9Point,Raster, and Cross
/* 090611 out
...
double scanTableGetOnValue(int index){
	if(seq.on == NULL){
		uM("scanTableGetOnValue(); WARNING: seq.on is NULL. I'll return 0.");
		return 0;
	}
	return seq.on[index];
}
double scanTableGetOffValue(int index){
	if(seq.off == NULL){
		uM("scanTableGetOffValue(); WARNING: seq.off is NULL. I'll return 0.");
		return 0;
	}
	return seq.off[index];
}
double scanTableGetLinePath(int index){
	if(seq.linePath == NULL){
		uM("scanTableGetLinePath(); WARNING: seq.linePath is NULL. I'll return 0.");
		return 0;
	}
