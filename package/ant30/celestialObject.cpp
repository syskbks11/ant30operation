#ifndef INCLUDE_GUARD_UUID_9a5630b7_cc8b_46c6_ae62_018bf8392dbb
#define INCLUDE_GUARD_UUID_9a5630b7_cc8b_46c6_ae62_018bf8392dbb
/*!
...
typedef struct {
  ...
  double velocity;         //!< 天体の視線速度 [m/s]//[km/s]くさい
...
int celestialObjectInit(){
	...
	if(confSetKey("Velocity"))
		p.velocity = atof(confGetVal())*1000;//091110 in
//		p.velocity = atof(confGetVal());
	...
	/**
	 * p.epochが他の値だとまずいのは富士通ライブラリのせい。ファニー。
	 */
//	if(p.sourceFlag == 0){//090111 in
		if(p.epoch == 0){//090610 in
			uM("Epoch is 0! I WILL SET IT TO 2!! check the table!");
			p.epoch = 2;
		}else if(p.epoch < 1 || p.epoch > 2){
/* 090610 out
...
//	}//090111 in
	...
	if(p.sourceFlag != 0 && p.coordinate != 0 ){
		if(p.sourceFlag == 99 && p.coordinate == 3){//090610 in
		}else{
