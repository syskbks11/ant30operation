...
/* 090616 moved to tParamTkb32clPort
  netClass_t* net;
*/
  tReq* req;
  tReq* reqRem;
...
/* 090616 moved to tkb32clPort.cpp
static int tkb32clFuncReq(tReq* req);
static int tkb32clFuncGetAns(tAns** ans);
*/
static int _tkb32clFuncReqObs(tReq* req, int reqFlg, tAns** ans, char* data);
static int _tkb32clFuncReqRemote(tReqRemote* rem, tAns** ans, char* data);
/* 090616 out
...
int tkb32clFuncInit(){
  ...
  memset(&p, 0, sizeof(p));
	int ret = tkb32clPortInit();//090616 in
	if(ret){
		uM("tkb32clPortInit() failed. ");
		return -1;
	}
/* 090616 moved to tkb32clPortInit()
  setParam();
...
int tkb32clFuncEnd(){
	tkb32clPortEnd();//090616 in
/* 090616 moved to tkb32clPortEnd()
  if(p.net){
    netclEnd(p.net);
    p.net = NULL;
  }
...
/*
  size = sizeof(p.req->size) + sizeof(p.req->reqVer) + sizeof(p.req->reqFlg) + sizeof(long) + pSize;
*/
...
/*
  ...
  p.req->reqFlg = 1;//initialize
  ((tReqInit*)&(p.req->data))->paramSize = pSize;
  strcpy( &((tReqInit*) &(p.req->data))->param, confGetAllKeyVal());
*/
  memset(req, 0, size);
  req->size = size;
...
*/
  ...
  //!Check
  ret = tkb32clPortGetAns(&p.ans);
  if(ret){
    uM1("tkb32clFuncReqInit(); tkb32clPortGetAns(); return %d", ret);
    return ret;
  }
  data = p.ans->data;
  if(data){
...
int tkb32clFuncReqStatus(){
  ...
  ret = tkb32clPortReq(p.req);
  if(ret){
    uM1("tkb32clFuncReqStatus(); tkb32clPortReq(); return %d", ret);
    return ret;
  }
  ...
  ret = tkb32clPortGetAns(&p.ans);
  if(ret < 0){
    uM1("tkb32clFuncReqStatus(); tkb32clPortGetAns(); return %d", ret);
    return ret;
  }
...
int tkb32clFuncReqPosoff(double* posoff){
  ...
  ret = _tkb32clFuncReqRemote(p.rem, &p.ans, &data);
  return ret;
}
...
int tkb32clFuncReqStartObs(){
  ...
  ret = _tkb32clFuncReqObs(p.req, 3, &p.ans, &data);//3:Start obs
  if(ret){
    uM1("tkb32clFuncReqStartObs(); _tkb32clFuncReqObs(); error %d", ret);
  }
  return ret;
}
...
int tkb32clFuncReqStopObs(){
  ...
  //!Request stop obs
  ret = _tkb32clFuncReqObs(p.req, 4, &p.ans, &data);//4:Stop obs
  if(ret){
    uM1("tkb32clFuncReqStopObs(); _tkb32clFuncReqObs(); error %d", ret);
  }
  return ret;
}
...
int tkb32clFuncReqEnd(){
  ...
  //!Request end
  ret = _tkb32clFuncReqObs(p.req, 5, &p.ans, &data);//5:End obs
  return ret;
}
...
int _tkb32clFuncReqObs(tReq* req, int reqFlg, tAns** ans, char* data){
  long size;
  int ret;
  ...
  ret = tkb32clPortReq(req);
  if(ret){
    uM2("_tkb32clFuncReqObs(); tkb32clPortReq(); reqFlg = %d ret = %d", reqFlg, ret);
    return ret;
  }

  ret = tkb32clPortGetAns(ans);
  if(ret){
    uM1("_tkb32clFuncReqObs(); tkb32clPortGetAns(); return %d", ret);
    return ret;
  }
  *data = (*ans)->data;
  if(*data){
    uM2("_tkb32clFuncReqObs(); ansFlg = %d ret = %d reject!!", (*ans)->ansFlg, *data);
    return (int)(*data);
  }
  else{
    uM1("_tkb32clFuncReqObs(); ansFlg = %d accept", (*ans)->ansFlg);
  }
  return 0;
}

int _tkb32clFuncReqRemote(tReqRemote* rem, tAns** ans, char* data){
  long size;
  int ret;
  ...
  ret = tkb32clPortReq(p.reqRem);
  if(ret){
    uM1("_tkb32clFuncReqRemote(); tkb32clPortReq(); ret = %d", ret);
    return ret;
  }

  ret = tkb32clPortGetAns(ans);
  if(ret){
    uM1("_tkb32clFuncReqRemote(); tkb32clPortGetAns(); return %d", ret);
    return ret;
  }
  *data = (*ans)->data;
  if(*data){
    uM2("_tkb32clFuncReqRemote(); ansFlg = %d ret = %d reject!!", (*ans)->ansFlg, *data);
    return (int)(*data);
  }
  else{
    uM1("_tkb32clFuncReqRemote(); ansFlg = %d accept", (*ans)->ansFlg);
  }
  return 0;
}
/* 090616 out
