...
*/
	...
	uM1("output mode is %d", outputMode);
        ...
        //printf("[DEBUG] Initializing penguin_signalG...\n");
	...
	if(ret){
        ...
        printf("penguin_signalG initialized successfully.\n");
	penguin_signalG_send_command("*RST");
	//gpibWdmWrite(if1Id, "*RST");
	//gpibWdmWrite(if1Id, "OUTP:STAT ON");

        //printf("[DEBUG] Initializing network server...\n");
	net = netsvInit(srvPort);
	if(net==NULL){
                perror("[ERROR] netsvInit failed");
                printf("[ERROR] netsvInit failed to initialize.\n");
		return -1;
        }
        printf("[INFO] Network server initialized successfully.\n");
	uEnd();
    while (1) {
        // 接続を待機
        //printf("[DEBUG] Before netsvWaiting\n");
        ret = netsvWaiting(net);
        //printf("[DEBUG] After netsvWaiting\n");
		if(ret < 0){
			continue;
		}
		uInit("../log/signalG/srvSignalG");//090813 in
	if (uInit("../log/signalG/srvSignalG") < 0) {
   	 printf("Failed to create srvSignalG-YYYYMMDDHHMMSS.log file.\n");
	}
/* 090813 out
