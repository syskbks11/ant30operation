...
int beReqInit(){
  ...
  for(int i = 0; i < p.BeNum; i++){
    ...
    uM1("debug beReqInit(); netclRead(); getSize=%ld", getSize);
    uM1("debug beReqInit(); netclRead(); getSize=%ld", p.ans[i].size);
    if(err == 2){
...
int beReqStart(int OnOffRoad, int returnFlag, unsigned long startTime,
  ...
  for(int i = 0; i < p.BeNum; i++){
    ...
    uM1("debug beReqStart(); netclRead(); getSize=%ld", getSize);
    uM1("debug beReqStart(); netclRead(); getSize=%ld", p.ans[i].size);
    if(err == 2){
      continue;
    }
    else if(err == 1){
...
int beReqState(){
  ...
  for(int i = 0; i < p.BeNum; i++){
    ...
    uM1("debug beReqState(); netclRead(); getSize=%ld", getSize);
    uM1("debug beReqState(); netclRead(); getSize=%ld", p.ans[i].size);
    if(err == 2){
      continue;
    }
    else if(err == 1){
